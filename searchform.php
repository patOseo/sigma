...
<form role="search" class="search-form" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>" <?php echo $aria_label; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped above. ?>>
	...
	<div class="input-group">
		<input type="search" class="field search-field form-control bg-transparent border-3 border-top-0 border-start-0 border-end-0 rounded-0 border-bottom text-white fw-500" id="<?php echo $uid; ?>" name="s" value="<?php the_search_query(); ?>" placeholder="<?php echo esc_attr_x( 'Search &hellip;', 'placeholder', 'understrap' ); ?>">
		<button type="submit" class="submit search-submit btn bg-transparent">
